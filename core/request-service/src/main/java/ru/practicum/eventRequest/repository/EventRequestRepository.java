...
public interface EventRequestRepository extends JpaRepository<EventRequest, Long> {
    ...
    @Query("SELECT COUNT(r) FROM EventRequest r WHERE r.eventId = :eventId and r.status = :status")
    long countRequestsByEventAndStatus(Long eventId, Status status);

    @Query("SELECT COUNT(r) FROM EventRequest r WHERE r.eventId = :eventId")
    long countRequestByEvent(Long eventId);
}
